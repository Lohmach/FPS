...
public class AI_enemy2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (global_variables.enemy2_hp <= 0)
        {
            Destroy(gameObject);
            global_variables.total_Points += From_enemy_to_player * 1.5f * global_variables.speed;
            global_variables.enemy2_hp = 1;
        }

        myTransform.rotation = Quaternion.Slerp(myTransform.rotation, Quaternion.LookRotation(traetrajectory[i].position - myTransform.position), 30 * Time.deltaTime);
        myTransform.position += myTransform.forward * 15 * Time.deltaTime * global_variables.speed;

        if (From_enemy_to_point < 10)
        {
            Destroy(gameObject);
        }
